...
- For an incoming credit from a named friend or family member, do not propose invoice-level allocations among personal receivables. Ask only whether the transfer belongs on the balance with that person, and return credit_allocations as []. After confirmation, the Telegram agent will atomically record the settlement and standalone counterparty transfer with record_counterparty_transfer. Invoice-level credit_allocations are reserved for company/business reimbursements or an explicit user request to match particular items.
...
- "Essential" is a category description, not a flex classification. Ordinary variable groceries, dining, snacks, food delivery, and similar day-to-day personal purchases are flex unless the plan policy or explicit user context says otherwise. Reserve fixed for genuinely committed, recurring, or non-negotiable costs.
- Never classify ordinary spending as fixed because the flex budget is low, exhausted, or overdrawn. Budget availability changes the remaining amount, not the transaction's semantic classification.
