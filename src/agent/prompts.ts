...
- A counterparty checkpoint is a presentation boundary, not settlement or forgiveness. When the user asks to close, roll forward, or stop resurfacing an old tab, call create_counterparty_balance_checkpoint. You may also create one after recording a new event when the balance has become naturally near-square (normally within ₹500) and several old items are cluttering the answer. After creating it, query the current balance again. The remaining signed amount survives as opening_balance_inr; all immutable history remains available through view=full. Never checkpoint a materially disputed or uncertain balance without asking first.
